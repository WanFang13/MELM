...
local function rescale_my(img, scale_factor)
	local maxLen = 1300
	if img:size(3)*scale_factor > maxLen or img:size(2)*scale_factor > maxLen then
		scale_factor = maxLen / math.max(img:size(3), img:size(2))
	end

	return image.scale(img, img:size(3) * scale_factor, img:size(2) * scale_factor)
end
...
function ExampleLoader:loadExample(exampleIdx, isTrainingPhase)
	...
	--JPG_NAME = self.dataset[o.subset]:getImageFileName(exampleIdx)
	if NEXT_BATCHIDX_FOR_VIS then JPG_NAME = self.dataset[o.subset]:getImageFileName(NEXT_BATCHIDX_FOR_VIS) end
	...
	return function(indexInBatch, batchTable)
		...
		for j, scale_ind in ipairs(scale_inds) do
			...
			--local img_scaled = scale_ind == 0 and img_original:clone() or rescale_my(img_original, scales[scale_ind])
