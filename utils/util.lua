...
function loadGtBoxes(im_name, cls)
	local xml = require 'xml'
	local imgNamesPath = 'data/datasets/VOCdevkit_2007/VOC2007/ImageSets/Main/trainval.txt'
	local file = io.open(imgNamesPath, "r")
	local imNames = {}
	for i = 1, 11540 do
		imNames[i] = file:read()
	end
	...
	local gtXMLPath = 'data/datasets/VOCdevkit_2007/VOC2007/Annotations/' .. im_name .. '.xml'
	local ann = xml.loadpath(gtXMLPath)
  ...
  local GtBoxes = nil
  for i = 1,#parsed.object do
  	if cls then
  		if parsed.object[i].name == classLabels[cls] then
  			local GtBox = torch.Tensor(1, 5):fill(0)
		  	GtBox[{1,1}] = parsed.object[i].bndbox.xmin
		  	GtBox[{1,2}] = parsed.object[i].bndbox.ymin
		  	GtBox[{1,3}] = parsed.object[i].bndbox.xmax
		  	GtBox[{1,4}] = parsed.object[i].bndbox.ymax
		  	GtBoxes = GtBoxes and torch.cat(GtBoxes, GtBox, 1) or GtBox
		  end
	  else
	  	local GtBox = torch.Tensor(1, 5):fill(0)
			GtBox[{1,1}] = parsed.object[i].bndbox.xmin
