...
function ORConv:__init(nInputPlane, nOutputPlane, nOrientation, kW, kH, dW, dH, padW, padH, groups)
    local delayedReset = self.reset
    self.reset = function() end
    parent.__init(self, nInputPlane, nOutputPlane, nOrientation, kW, kH, dW, dH, padW, padH)
    self.reset = delayedReset
    self.groups = groups or 1
    assert((nInputPlane * nOrientation) % self.groups == 0,
           'nInputPlane x nOrientation should be divisible by nGroups')
    assert((nOutputPlane * nOrientation) % self.groups == 0,
           'nOutputPlane x nOrientation should be divisible by nGroups')
    self.shareWeight = torch.Tensor(nOutputPlane * self.nOrientation, nInputPlane * self.nOrientation / self.groups, self.kH, self.kW)
    self.gradShareWeight = torch.Tensor(nOutputPlane * self.nOrientation, nInputPlane * self.nOrientation / self.groups, self.kH, self.kW)
    self:reset()
...
function ORConv:resetShareWeightDescriptors()
    ...
    -- create filterDescriptor for shareWeight
    self.shareWeightDesc = ffi.new('struct cudnnFilterStruct*[1]')
    errcheck('cudnnCreateFilterDescriptor', self.shareWeightDesc)
    local desc = torch.IntTensor({self.nOutputPlane*self.nOrientation/self.groups,
                              self.nInputPlane*self.nOrientation/self.groups,
                              self.kH, self.kW})
    ...
    if self.bias then
        self.biasDesc = cudnn.toDescriptor(self.bias:view(1, self.nOutputPlane * self.nOrientation,1,1))
    end
end
...
function ORConv:createIODescriptors(input)
    ...
    or input:size(3) ~= self.iSize[3] or input:size(4) ~= self.iSize[4] then
        ...
        assert(self.nInputPlane * self.nOrientation == input:size(2), 'input has to contain: '
        ...
        local input_slice = {{},{1,self.nInputPlane * self.nOrientation/self.groups},{},{}}
        ...
        local output_slice = {{},{1,self.nOutputPlane * self.nOrientation/self.groups},{},{}}
        ...
        local algWorkspaceLimit = self.workspace_limit
            or (self.nInputPlane * self.nOrientation * self.kH * self.kW * 4) -- 4 = sizeof int/float.
        ...
        self.input_offset = self.nInputPlane * self.nOrientation / self.groups * iH * iW
        self.output_offset = self.nOutputPlane * self.nOrientation / self.groups * oH * oW
        self.shareWeight_offset = self.nInputPlane * self.nOrientation / self.groups * self.nOutputPlane * self.nOrientation / self.groups * kH * kW
...
function ORConv:updateOutput(input)
    ...
    -- generate rotated versions on the fly
    input.salc.ARF_Rotation(
      self,
      self.weight,
      self.indices,
      self.shareWeight,
      self.kW, self.kH, self.nInputPlane, self.nOutputPlane, self.nOrientation
    )
...
function ORConv:accGradParameters(input, gradOutput, scale)
    ...
    -- align gradWeight for different rotated versions and sum up
    input.salc.ARF_Align(
      self,
      self.gradWeight,
      self.indices,
