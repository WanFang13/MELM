local ADEntropy, Parent = torch.class('nn.ADEntropy', 'nn.Module')
...
function ADEntropy:updateOutput(input)
  --input[1] : 1    * 20 image class score
  --input[2] : nRoi * 21 boxes score det score
  --input[2] : nRoi * 21 boxes score det score
  ...
  if self.train then
    ...
    local overlaps_temp = GetOverlap(boxes_score_det[1][{{},{1,20}}], rois, gt_image_label)
    --get labels and max_overlaps
    local overlaps2 = torch.cmax(overlaps_temp, overlaps)
    local max_overlap, max_inds = overlaps2:max(2)
    local labels = GetLabels(max_overlap, max_inds, gt_image_label)
    --get boxes inds
    ...
    self.output = {}
    self.output[1] = im_score
    self.output[2] = ScoreOutput:view(-1,20)
...
function ADEntropy:updateGradInput(input, gradOutput)
  --input[1] : 1    * 20 image class score
  --input[2] : nRoi * 21 boxes score det score
  local im_score        = input[1]
  local rois            = scale0_rois[1]:cuda() --nRoi * 5 dim
